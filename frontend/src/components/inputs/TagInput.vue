<template>
  <div class="flex flex-col gap-1.5">
    ...
    <div
      class="flex flex-wrap items-center gap-1.5 rounded-lg border border-outline-gray-2 bg-surface-base px-2 py-1.5 transition focus-within:border-outline-gray-3 focus-within:ring-2 focus-within:ring-outline-gray-3 motion-reduce:transition-none"
      @click="focusInput"
    >
      <span
        v-for="(tag, index) in modelValue"
        :key="`${index}-${tag}`"
        class="inline-flex items-center gap-1 rounded-md bg-surface-gray-2 py-0.5 pl-2 pr-0.5 text-sm text-ink-gray-8"
      >
        {{ tag }}
        <button
          type="button"
          class="flex size-5 items-center justify-center rounded text-ink-gray-6 transition hover:bg-surface-gray-3 hover:text-ink-gray-9 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-outline-gray-3 motion-reduce:transition-none"
